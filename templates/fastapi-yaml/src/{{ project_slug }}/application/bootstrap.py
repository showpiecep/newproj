...
def bootstrap(config_path: str | Path = "config.yaml") -> Application:
    """Generate the config template and schema, load config and assemble the application."""

    config_path = Path(config_path)
    default_template_path = config_path.parent / AppSettings().config_template_path
    write_config_artifacts(Config, default_template_path)
    ...
    if configured_template_path != default_template_path:
        write_config_artifacts(Config, configured_template_path)
